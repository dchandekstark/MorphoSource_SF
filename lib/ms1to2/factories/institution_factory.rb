	...
	module Factories
		class InstitutionFactory < Factory
			...
			def process_facility_table
				facility_table = ms1_input_data.public_send(:ms_facilities)
				facility_table.each do |k, v|
			...
			def matching_institution(institution_name)
				if model.where({:title => institution_name}).first
					model.where({:title => institution_name}).first.id
				elsif ms2_output_data.find_ids(ms2_model_var, :title, institution_name).first
					ms2_output_data.find_ids(ms2_model_var, :title, institution_name).first
				else
					ms2_table.each do |k, v|
						return if v.key?(:title) && v[:title] == institution_name
					end
					nil
				end
			end
